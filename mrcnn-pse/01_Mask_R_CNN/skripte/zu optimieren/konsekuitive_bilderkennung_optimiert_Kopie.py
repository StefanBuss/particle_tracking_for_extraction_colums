...
#list_verwendete_daten = ["20221124_Oben_1,2_1_40"]

list_konti_dispers_hold_up = {"test": [0.1876 / 60000, 0.1574 / 60000, 7.5 / 100], "20221124_Oben_1,2_1_40": [0.1876 / 60000, 0.1574 / 60000, 7.5 / 100] , "20221124_Oben_1,2_1_80": [0.371 / 60000, 0.31 / 60000, 12.6 / 100],
...
def load_data():
    ...
    return list_r, list_elem, len(list_elem)-1
...
def plot_rgb_v_y_gekuertzt(list_v_y, zeit):
    ...
    ax1.plot(np.linspace(0, zeit[i], len(zeit[:i])), fittet_rgb, 'b-', label = "fittet rgb")
    ...
    ax2.plot(np.linspace(0, zeit[i], len(zeit[:i])), fittet_v_y, 'k-', label = "fitted v_y")
...
def plot_rgb_v_y():
    ...
    ax1.plot(np.linspace(0, zeit[-1], len(zeit[:-1])), fittet_rgb, 'b-', label = "fittet rgb")
    ax1.set_xlabel('Zeit [s]')
    ax1.set_ylabel('RGB', color='b')
    ax1.tick_params('y', colors='b')
    ax1.tick_params(direction="in")
    ...
    ax2.plot(np.linspace(0, zeit[-1], len(zeit[:-1])), fittet_v_y, 'k-', label = "fitted v_y")
...
def berechne_verhältnis_anzahl_tropfen():
    #größenverteilung_bild_zu_bild() ->  list_größenverteilung_bild_zu_bild         anzahl_tropfen_von_bild_zu_bild 

    #size_distribution() -> M_dis = pickle.load(open(ROOT_DIR+"\\datasets\\output_analyseddata\\"+VERWENDETE_DATEN+"\\M_dis", "rb"))
    #anzahl_tropfen = list(M_dis[:,1])
    ...
    for i, größe in enumerate(step_list):
        x.append(str(np.round(größe - step_size, 2)) + " bis " + str(größe))
    """
    if 0 not in anzahl_tropfen[:-1]:
        verhältnis = np.array(list_größenverteilung_bild_zu_bild)/np.array(anzahl_tropfen[:-1])
        plot_bar(loc_label = "upper left",label = "anzahl durch anzahl", pos_string = None, strings = [], x_achse = x, y_achse = verhältnis, y_modell = None, y_label = "anzahl durch anzahl", x_label = "Tropfengröße [mm]", title = 'Anzahl Durch Anzahl '+ VERWENDETE_DATEN, SAVE_DIR = ROOT_DIR + "\\datasets\\output_analyseddata\\" + VERWENDETE_DATEN + "\\anzahl_durch_anzahl"+VERWENDETE_DATEN)
    else:
        print("funktion berechne_verhältnis_anzahl_tropfen wurde nicht ausgeführt, da in einer tropfenklasse keine Tropfen erkannt wurden")
    """ 
    verhältnis = np.array(list_größenverteilung_bild_zu_bild)/np.array(anzahl_tropfen[:-1])
    plot_bar(loc_label = "upper left",label = "anzahl durch anzahl", pos_string = None, strings = [], x_achse = x, y_achse = verhältnis, y_modell = None, y_label = "anzahl durch anzahl", x_label = "Tropfengröße [mm]", title = 'Anzahl Durch Anzahl '+ VERWENDETE_DATEN, SAVE_DIR = ROOT_DIR + "\\datasets\\output_analyseddata\\" + VERWENDETE_DATEN + "\\anzahl_durch_anzahl"+VERWENDETE_DATEN)
...
python C:\\Users\\stefa\\Desktop\\BA_git\\mrcnn-pse\\01_Mask_R_CNN\\skripte\\konsekuitive_bilderkennung_optimiert_Kopie.py
