...
def konsekutive_bilderverfolgung():
    dic_id, id_counter, list_alle_potentiellen_vorgänger_nachfolger_beziehungen = {}, 0, []
    os.mkdir(ROOT_DIR + "\\datasets\\output_images\\" + VERWENDETE_DATEN)
    list_komb = []

    #dic_droplet = list_alle_potentiellen_vorgänger_nachfolger_beziehungen
    ...
    for bild_index in range(start, ende):
        ...
        for tropfen_index_betrachtetes_bild in range(len(alle_tropfen_betrachtetes_bild)):
            list_alle_potentiellen_vorgänger_nachfolger_beziehungen.append([[bild_index, tropfen_index_betrachtetes_bild]])
        ...
        for tropfen_index_betrachtetes_bild in range(len(alle_tropfen_betrachtetes_bild)):
            ...
            for tropfen_index_naechstest_bild in range(len(alle_tropfen_naechstest_bild)):
                ...
                if Bedingung_vorgaenger_nachfolger(morph_aktuelles_bild, morph_naechstest_bild, alle_tropfen_betrachtetes_bild, alle_tropfen_naechstest_bild, tropfen_index_betrachtetes_bild, tropfen_index_naechstest_bild) == True:
                    for i, value in enumerate(list_alle_potentiellen_vorgänger_nachfolger_beziehungen):
                        if value[0][0] == bild_index and value[0][1] == tropfen_index_betrachtetes_bild:
                            list_alle_potentiellen_vorgänger_nachfolger_beziehungen[i].append([bild_index + 1, tropfen_index_naechstest_bild])
            
        List_second = [] 
        dic_droplet_neu = []
        for i, elem in enumerate(list_alle_potentiellen_vorgänger_nachfolger_beziehungen):               
            if len(elem) > 2:
        ...
        list_alle_potentiellen_vorgänger_nachfolger_beziehungen = dic_droplet_neu + list_tasaelicher_nachfolger
        ...
        #hier wird der wahrscheinlichste vorgänger ausgewählt wenn mehrere potentille vorgänger für einen nachfolger in frage kommen.
        list_wahrscheinlichste_vorgänger_nachfolger_beziehung = []
        for i, nachfolger in enumerate(List):
            List_potentieller_vorgaenger = []
            for ii, pair in enumerate(list_alle_potentiellen_vorgänger_nachfolger_beziehungen):
            ...
            list_wahrscheinlichste_vorgänger_nachfolger_beziehung.append([vorgaenger, nachfolger])

        list_komb.append(list(list_wahrscheinlichste_vorgänger_nachfolger_beziehung))

        if dic_id == {}:
            for i, val in enumerate(list_wahrscheinlichste_vorgänger_nachfolger_beziehung):
