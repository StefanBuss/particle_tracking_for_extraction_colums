...
def konsekutive_bilderverfolgung():
    ...
    list_komb = []
    for bild_index in range(start, ende):
        list_tatsaechlicher_vorgaenger = pickle.load(open(ROOT_DIR+"\\daten\\"+str(bild_index) + VERWENDETE_DATEN, "rb"))
            #list_tatsaechlicher_vorgaenger.append([vorgaenger, nachfolger])

        print(list_tatsaechlicher_vorgaenger)
        list_komb.append(list(list_tatsaechlicher_vorgaenger))
        ...
        if dic_id == {}:
            for i, val in enumerate(list_tatsaechlicher_vorgaenger):
            ...
            for i, val in enumerate(list_tatsaechlicher_vorgaenger):
                ...
                for elem in dic_id: 
                    #einaml schon enthalten -> zu bekannter id hinzufügen 
                    if dic_id[elem][-1] == val[0]:
                        dic_id[elem].append(val[1])
                        found = True
                            
                #noch nicht enthalten -> neue id erstellen
                if found == False:
                    dic_id[id_counter] = val
                    id_counter += 1  
