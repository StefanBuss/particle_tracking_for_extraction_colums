...
#definition welche Datensätze analysiert werden sollen
...
def plot_modell_wiederholder(y1,y2):
    ...
    ax.set_title(VERWENDETE_DATEN, fontsize = label_fontsize, fontname = "Arial")
...
def verhaeltis_erkannte_bilder():
    ...
    plot_funktion(zeit[:-1], [verhaeltnis], "Anzahl konsekutiv verfolgter Tropfen/ \n Anzahl druch mrcnn detektierter Tropfen", "Zeit [s]", ROOT_DIR + "\\datasets\\output_analyseddata\\" + VERWENDETE_DATEN + "\\verhaeltis_" + VERWENDETE_DATEN, 0, 1, VERWENDETE_DATEN)    
...
def plot_bar(loc_label,label, pos_string,strings, x_achse, y_achse, y_modell, y_label, x_label, title, SAVE_DIR):
    ...
    if all(element in list_verwendete_daten for element in x_achse):
...
def plot_size_distribution(): 
    ...
    plot_bar(loc_label = None, label = None,pos_string = [0.65,0.93], strings = [], x_achse = x, y_achse = anzahl_tropfen, y_modell =  None, y_label = 'Anzahl ID aus dic_id', x_label = 'Tropfengröße [mm]', title = VERWENDETE_DATEN + " summe anzahl ID aus dic_id: " + str(sum(anzahl_tropfen)),SAVE_DIR = ROOT_DIR + "\\datasets\\output_analyseddata\\" + VERWENDETE_DATEN + "\\v_y_positive_negative_all_anzahl_tropfen\\anzahl_tropfen_"+VERWENDETE_DATEN)
...
def plot_geschwindigkeitsverteilung(dict, größe, max_anzahl):
    ...
    ax.set_ylabel('Anzahl ID aus dic_id', fontsize = 24, fontname = "Arial")
    ax.set_title(VERWENDETE_DATEN + ", " + p_text + ", " + "Summe ID: " + str(gesamt_anzahl_tropfen), fontsize = 24, fontname = "Arial")
...
def plot_rgb_v_y_gekuertzt(list_v_y, zeit):
    ...
    ax2.set_title(VERWENDETE_DATEN, fontsize = label_fontsize, fontname = "Arial")
...
def plot_phasenverschiebung():
    ...
    plot_bar(loc_label = "upper left", label = None, pos_string = [], strings = [], x_achse = list_verwendete_daten, y_achse = list_phasenverschiebung_in_degree, y_modell = None, y_label = 'Phasenverschiebung [°]', x_label = '',title = "Phasenverschiebung", SAVE_DIR =ROOT_DIR+"\\datasets\\output_analyseddata\\phasenverschiebung.png")
...
#ACHTUNG ALTE FUNKTION DIE GELÖSCHT WERDEN KANN.
def parametrisiere_modell_ALTE_PARAMETRISIERUNGSFUNTKION():
    summe_oberezeile = 0
    summe_unterezeile = 0
    os.mkdir(ROOT_DIR + "\\datasets\\output_analyseddata\\" + VERWENDETE_DATEN + "\\v_y_positive_negative_all_anzahl_tropfen")
...
#ACHTUNG ALTE FUNKTION DIE GELÖSCHT WERDEN KANN.
def gesamt_parametrisierung_ALTE_PARAMETRISIERUNGSFUNTKION():
...
def länge_eine_folge():
    ...
    plot_bar(loc_label = "upper left", label = None, pos_string = None, strings = [], x_achse = x, y_achse = länge_list[0:-1], y_modell = None, y_label = "Durchschnittliche Länge \n einer ID aus dic_id", x_label = "Tropfengröße [mm]", title = VERWENDETE_DATEN, SAVE_DIR = ROOT_DIR + "\\datasets\\output_analyseddata\\" + VERWENDETE_DATEN + "\\länge_ids_"+VERWENDETE_DATEN)
...
def laenge_datensatz():
    ...
    plot_bar(loc_label = "upper left",label = None, pos_string = None, strings = [], x_achse = list_verwendete_daten, y_achse = list_zeitdifferenz, y_modell = None, y_label = "Zeit [ms]", x_label = "", title = "Länge der Datensätze", SAVE_DIR = ROOT_DIR+"\\datasets\\output_analyseddata\\länge_datensätze.png")
...
def exponent_parametrisierung():
    ...
    for i, VERWENDETE_DATEN in enumerate(list_verwendete_daten):
        if VERWENDETE_DATEN.split("_")[4] != "100":
            ...
            list_n.append(n)
    plot_bar(loc_label = "upper left",label = None, pos_string = None, strings = [], x_achse = list_plot_exponent, y_achse = list_n, y_modell = None, y_label = "Schwarmexponent", x_label = "", title = "Schwarmexponent von Datensätzen", SAVE_DIR = ROOT_DIR+"\\datasets\\output_analyseddata\\exponent_n.png")
...
def berechnezeit_differenz():
    if konstante_framerate == False:
        ...
        plot_funktion(np.linspace(0, len(zeit[:-1]), len(zeit[:-1])), [list_zeitdifferenz], "Zeitdifferenz zum vorherigen Bild [ms]", "Bild_Index", ROOT_DIR + "\\datasets\\timestamps\\timestamps_" + VERWENDETE_DATEN + ".png", None, None, VERWENDETE_DATEN)
        plot_funktion(np.linspace(0, len(zeit), len(zeit)), [zeit, np.linspace(0, 10, len(zeit)) ], "Zeit in [s]", "Bild_Index", ROOT_DIR + "\\datasets\\timestamps\\zeit_" + VERWENDETE_DATEN + ".png", None, None, VERWENDETE_DATEN)
...
#das ist die main funktion die definiert welcher teil des algorithmus laufen soll.
...
python C:\\Users\\StefanvenderBuss\\Desktop\\BA_git\\mrcnn-pse\\01_Mask_R_CNN\\skripte\\konsekuitive_bilderkennung.py
